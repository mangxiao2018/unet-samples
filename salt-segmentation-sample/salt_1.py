...
def convert_data():
    ...
    a,b = get_data(path_train, train=True)
    X,y = a,b
    global X_train, X_valid, y_train, y_valid
    c,d,e,f = train_test_split(X, y, test_size=0.15, random_state=2018)
    X_train, X_valid, y_train, y_valid = c,d,e,f
    ...
    ax[1].set_title('Salt');
    fig, ax = plt.subplots(1, 2, figsize=(20, 10))
    ax[0].imshow(X_train[ix, ..., 0], cmap='Greens', interpolation='bilinear')
    if has_mask:
        ax[0].contour(y_train[ix].squeeze(), colors='k', levels=[0.5])
    ax[0].set_title('Seismic')
    ax[1].imshow(y_train[ix].squeeze(), interpolation='bilinear', cmap='gray')
...
callbacks = [
        ...
        ModelCheckpoint('model-tgs-saltx.h5', verbose=1, save_best_only=True, save_weights_only=True)
    ]
